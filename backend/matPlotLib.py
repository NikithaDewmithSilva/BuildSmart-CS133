...
def load_data():
    ...
    try:
        response = supabase.table("material_usage").select("*").order("created_at", desc=True).limit(1).execute()
...
def generate_category_chart():
    ...
    categories = [cat for cat in categories if cat != "Summary" and not cat.endswith("Usage Tracking")]

    material_counts = [len(data[category]) for category in categories]

    short_categories = [cat.replace(' Material Estimate', '') for cat in categories]
...
def generate_material_chart(material_type):
    ...
    remaining_values = [est - used for est, used in zip(estimated_values, used_values)]
    
    plt.figure(figsize=(10, 6))
    
    if not categories:
        plt.text(0.5, 0.5, f"No {material_type} data found", ha='center', va='center')
    else:
        bar_width = 0.35
        x = range(len(categories))
        
        plt.bar(x, estimated_values, bar_width, color=material_map[material_type]['color'], alpha=0.7, label='Estimated')
        plt.bar([i + bar_width for i in x], used_values, bar_width, color='green', alpha=0.7, label='Used')
        
        plt.xlabel('Construction Categories')
        plt.ylabel(f'{material_map[material_type]["title"]} ({material_map[material_type]["unit"]})')
        plt.title(f'{material_map[material_type]["title"]} - Estimated vs Used')
